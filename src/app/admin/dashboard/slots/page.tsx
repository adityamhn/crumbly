  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-amber-900">Delivery Slots</h1>
        <button
      ...
      <div className="grid gap-3">
        {slots.map(slot => (
          <div key={slot.id} className="bg-white rounded-2xl p-4 shadow-sm flex items-center gap-4">
            <div className="flex-1">
              <p className="font-semibold text-amber-900">{formatDate(slot.date)}</p>
              <p className="text-sm text-amber-600">{formatTime(slot.start_time)} - {formatTime(slot.end_time)}</p>
              <p className="text-xs text-amber-500 mt-1">{slot.current_orders} / {slot.max_orders} orders</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setEditing(slot)} className="px-3 py-1 rounded-lg text-xs font-medium bg-amber-100 text-amber-700 hover:bg-amber-200">
                Edit
              </button>
        ...
        {slots.length === 0 && (
          <p className="text-center py-8 text-amber-500">No delivery slots. Add one to start accepting orders!</p>
        )}
      </div>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            <h2 className="text-lg font-bold text-amber-900 mb-4">{editing.id ? 'Edit Slot' : 'New Slot'}</h2>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-amber-800 mb-1">Max Orders</label>
                <input
                  type="number"
