  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-pink-900">Menu Items</h1>
        <button
      ...
      <div className="grid gap-4">
        {items.map(item => (
          <div key={item.id} className="bg-white rounded-2xl p-4 shadow-sm flex items-center gap-4">
            {item.image_url ? (
              ...
              <div className="w-16 h-16 rounded-xl bg-pink-100 flex items-center justify-center text-pink-400 text-xs">No img</div>
            )}
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-pink-900">{item.name}</h3>
              <p className="text-sm text-pink-600 truncate">{item.description}</p>
              <p className="text-sm font-medium text-pink-800">₹{item.price} · Qty: {item.available_quantity}</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setEditing(item)} className="px-3 py-1 rounded-lg text-xs font-medium bg-pink-100 text-pink-700 hover:bg-pink-200">
                Edit
              </button>
        ...
        {items.length === 0 && (
          <p className="text-center py-8 text-pink-500">No menu items yet. Add your first item!</p>
        )}
      </div>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold text-pink-900 mb-4">{editing.id ? 'Edit Item' : 'New Item'}</h2>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-pink-800 mb-1">Description</label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-pink-800 mb-1">Price (₹)</label>
                  <input
                    type="number"
                    value={editing.price || ''}
                ...
                <div>
                  <label className="block text-sm font-medium text-pink-800 mb-1">Quantity</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-pink-800 mb-1">Sort Order</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-pink-800 mb-1">Image</label>
